...
/*
void computeManyKinematicsForDVCSComptonFormFactorWithReplica() {
    ...
    // Create CFF module with the BaseModuleFactory
    PARTONS::DVCSConvolCoeffFunctionModule* pDVCSCFFANN =
    ...
    for (size_t i = 0; i < 101; i++) {
                        ...
                        static_cast<DVCSCFFNN*>(pDVCSCFFANN)->configure(
                                ElemUtils::Parameter(
                                        DVCSCFFNN::PARAMETER_NAME_REPLICA, i));

                        // Run computation
                        PARTONS::List<PARTONS::DVCSConvolCoeffFunctionResult> cffResultList =
                                pDVCSConvolCoeffFunctionService->computeForOneCCFModelAndManyKinematics(
                                        cffKinematicList, pDVCSCFFANN);

                        //Print Replica Number
                        std::cout << "Replica number " << i << std::endl ;
