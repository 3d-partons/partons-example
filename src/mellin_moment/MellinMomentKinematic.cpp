...
const std::string MellinMomentKinematic::MELLIN_MOMENT_KINEMATIC_PARAMETER_NAME_N = "n";
...
MellinMomentKinematic::MellinMomentKinematic() :
        Kinematic("MellinMomentKinematic"), m_kinematicType(KinematicType::THEO), m_n(
                1.), m_xi(0.), m_t(0.), m_MuF2(0.), m_MuR2(0.) {
}
...
MellinMomentKinematic::MellinMomentKinematic(int n, double xi, double t, double MuF2,
        double MuR2) :
        Kinematic("MellinMomentKinematic"), m_kinematicType(KinematicType::THEO), m_n(n), m_xi(
                xi), m_t(t), m_MuF2(MuF2), m_MuR2(MuR2) {
}

MellinMomentKinematic::MellinMomentKinematic(const ElemUtils::GenericType& n,
        ...
        Kinematic("MellinMomentKinematic"), m_kinematicType(KinematicType::THEO), m_n(
                n.toInt()), m_xi(xi.toDouble()), m_t(t.toDouble()), m_MuF2(
...
MellinMomentKinematic::MellinMomentKinematic(const ElemUtils::Parameters &parameters) :
    ...
    if (parameters.isAvailable(MellinMomentKinematic::MELLIN_MOMENT_KINEMATIC_PARAMETER_NAME_N)) {
        m_n = parameters.getLastAvailable().toDouble();
    } else {
        errorMissingParameter(MellinMomentKinematic::MELLIN_MOMENT_KINEMATIC_PARAMETER_NAME_N);
    }
...
void MellinMomentKinematic::updateHashSum() const {
    setHashSum(
            Partons::getInstance()->getServiceObjectRegistry()->getCryptographicHashService()->generateSHA1HashSum(
                    ElemUtils::Formatter() << m_n << m_xi << m_t << m_MuF2
                            << m_MuR2));
}
...
double MellinMomentKinematic::getN() const {
    return m_n;
}

void MellinMomentKinematic::setN(int n) {
    m_n = n;
    updateHashSum();
}
...
void MellinMomentKinematic::serialize(ElemUtils::Packet &packet) const {
    ...
    packet << m_n << m_xi << m_t << m_MuF2 << m_MuR2;
}
