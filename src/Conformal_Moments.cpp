...
ConformalMoments::~ConformalMoments(){
  ...
  PARTONS::Partons::getInstance()->getModuleObjectFactory()->updateModulePointerReference(m_pActiveFlavorsThresholdsModule, 0);
  PARTONS::Partons::getInstance()->getModuleObjectFactory()->updateModulePointerReference(m_pRunningAlphaStrongModule, 0);
  PARTONS::Partons::getInstance()->getModuleObjectFactory()->updateModulePointerReference(m_pGPDEvolutionModel, 0);
  PARTONS::Partons::getInstance()->getModuleObjectFactory()->updateModulePointerReference(m_pGPDModel, 0);
...
void ConformalMoments::setPARTONS()
{
  ...
  m_pGPDModel =  PARTONS::Partons::getInstance()->getModuleObjectFactory()->newGPDModule(PARTONS::simpleRDDAModel::classId);

  //Create GPD evolution module with the BaseModuleFactory
  m_pGPDEvolutionModel = PARTONS::Partons::getInstance()->getModuleObjectFactory()->newGPDEvolutionModule(PARTONS::GPDEvolutionApfel::classId);

  //Choose Alpha_Strong Module
  m_pRunningAlphaStrongModule = PARTONS::Partons::getInstance()->getModuleObjectFactory()->newRunningAlphaStrongModule(PARTONS::RunningAlphaStrongApfel::classId);

  // Create active flavors thresholds module with the BaseModuleFactory
  m_pActiveFlavorsThresholdsModule = PARTONS::Partons::getInstance()->getModuleObjectFactory()->newActiveFlavorsThresholdsModule(PARTONS::ActiveFlavorsThresholdsConstant::classId);
...
void ConformalMoments::setEvolution()
{
  ...
  // of steps in the integration over factorization scale) One step is
  // a typical value for Vinnikov code
  ...
  // PerturbativeQCD = LO
  parameters.add(PARTONS::PerturbativeQCDOrderType::PARAMETER_NAME_PERTURBATIVE_QCD_ORDER_TYPE,
  ...
  // Grid parameters
  parameters.add(PARTONS::GPDEvolutionApfel::PARAM_NAME_SUBGRID_NODES, "100 100 100 100" );
  parameters.add(PARTONS::GPDEvolutionApfel::PARAM_NAME_SUBGRID_LOWER_BOUNDS, "1e-7 1e-1 5e-1 9e-1" );
...
void ConformalMoments::ComputeConformalMoments()
{
  ...
  //Exporting results
  const std::vector<double> mu2v{pow(1, 2), pow(2, 2), pow(5, 2), pow(10, 2), pow(100, 2)};

  const int nxi = 50;
  const double ximin = 1e-3;
  const double ximax = 0.9;
  const double xistp = ( ximax - ximin ) / ( nxi - 1 );
  ...
  std::vector<double> err_Para = {1e-5};// last element of parameters is  target error of  integration routine

  //Preparing output file
  ElemUtils::FileUtils* outputFile;
  std::ofstream evolutionOutputFile_gegenbauerModel;
  outputFile->open( evolutionOutputFile_gegenbauerModel, "/Users/vb262523/Codes/PARTONS/partons-example/ConformalMomentC4.dat", std::ios_base::out );
  ...
  */
  ...
  for (double m_tmu2 : mu2v)
      ...
      for ( double m_txi = ximin; m_txi <= ximax; m_txi += xistp )
	{
	  ...
	  outputFile->write( evolutionOutputFile_gegenbauerModel,
			     ElemUtils::Formatter() << std::scientific << std::to_string(m_xi)
			     << "\t" << std::to_string(m_mu2)
			     << "\t" << dummyConfMom
			     ...
			     << "\t" << GammaERBL
			     << "\n");
	  std::cout << std::scientific << std::to_string(m_xi)
		    << "\t" << std::to_string(m_mu2)
		    << "\t" << dummyConfMom
		    << "\t" << refConfMom.at(iConfMom)
		    << "\t" << 1. - dummyConfMom / ( GammaERBL * refConfMom.at(iConfMom) )
